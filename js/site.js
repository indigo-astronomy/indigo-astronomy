(function () {
	...
	// the driver and agent totals in the text are taken from the tables themselves
	function showTotal(selector, tableId) {
		var table = document.getElementById(tableId);
		if (!table) return;
		var total = table.querySelectorAll("tbody tr").length;
		var nodes = document.querySelectorAll(selector);
		for (var i = 0; i < nodes.length; i++) nodes[i].textContent = total;
	}
	showTotal(".driver-total", "driver-table");
	showTotal(".agent-total", "agent-table");

	var search = document.getElementById("driver-search");
	if (search) {
		// only the driver table is searched, the agents table is left alone
